...
async def leave_a_chat(bot, message):
    ...
    try:
        buttons = [[InlineKeyboardButton('☣️ 𝗦𝘂𝗽𝗽𝗼𝗿𝘁', url=f'https://t.me/{SUPPORT_CHAT}')]]
...
async def get_ststs(bot, message):
    rju = await message.reply('<b>Patientez....</b>')
...
async def list_users(bot, message):
    sps = await message.reply('Obtention de la liste des utilisateurs...')
    users = await db.get_all_users()
    out = "Les utilisateurs enregistrés dans la base de donnée :\n\n"
...
async def list_chats(bot, message):
    sps = await message.reply('Obtention de la liste des discussions...')
    chats = await db.get_all_chats()
    out = "Les discussions sauvegardés dans la base de données sont :\n\n"
    async for chat in chats:
        username = chat['username']
        username = "private" if not username else "@" + username
        out += f"**- Titre :** `{chat['title']}`\n**- ID:** `{chat['id']}`\n**𝗡𝗼𝗺 𝗱'𝘂𝘁𝗶𝗹𝗶𝘀𝗮𝘁𝗲𝘂𝗿:** {username}\n"
    try:
        await sps.edit_text(out)
    except MessageTooLong:
        with open('chats.txt', 'w+') as outfile:
            outfile.write(out)
        await message.reply_document('chats.txt', caption="Liste des discussions")
...
async def show_id(client, message):
    ...
    if chat_type == enums.ChatType.PRIVATE:
        ...
        await message.reply_text(f"<b>💠 𝗣𝗿𝗲𝗻𝗼𝗺:</b> {first}\n<b>💠 𝗡𝗼𝗺:</b> {last}\n<b>💠 𝗡𝗼𝗺 𝗱'𝘂𝘁𝗶𝗹𝗶𝘀𝗮𝘁𝗲𝘂𝗿:</b> {username}\n<b>💠 𝗜𝗗 𝗧𝗲𝗹𝗲𝗴𝗿𝗮𝗺:</b> <code>{user_id}</code>\n<b>💠 𝗜𝗗 𝗧𝗶𝗲𝗿𝘀:</b> <code>{dc_id}</code>", quote=True)

    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        _id = ""
        _id += f"<b>💠 𝗜𝗗 𝗱𝗲 𝗹𝗮 𝗱𝗶𝘀𝗰𝘂𝘀𝘀𝗶𝗼𝗻</b>: <code>{message.chat.id}</code>\n"

        if message.reply_to_message:
            _id += (
                "<b>💠 𝗜𝗗 𝗱𝗲 𝗹'𝗨𝘁𝗶𝗹𝗶𝘀𝗮𝘁𝗲𝘂𝗿</b>: "
...
async def user_info(client, message):
    status_message = await message.reply_text("`Patientez....`")
    from_user = None
    ...
    if from_user is None:
        return await status_message.edit("Identifiant invalide / Message non spécifié")
    message_out_str = ""
    message_out_str += f"<b>💠 𝗣𝗿𝗲𝗻𝗼𝗺:</b> {from_user.first_name}\n"
    last_name = from_user.last_name or "<b>ɴᴏɴᴇ</b>"
    message_out_str += f"<b>💠 𝗡𝗼𝗺:</b> {last_name}\n"
    message_out_str += f"<b>💠 𝗜𝗗 𝗧𝗲𝗹𝗲𝗴𝗿𝗮𝗺:</b> <code>{from_user.id}</code>\n"
    username = from_user.username or "<b>ɴᴏɴᴇ</b>"
    dc_id = from_user.dc_id or "[ᴜꜱᴇʀ ᴅᴏꜱᴇ'ᴛ ʜᴀᴠᴇ ᴀ ᴠᴀʟɪᴅ ᴅᴩ]"
    message_out_str += f"<b>💠 𝗜𝗗 𝗧𝗶𝗲𝗿𝘀:</b> <code>{dc_id}</code>\n"
    message_out_str += f"<b>💠 𝗡𝗼𝗺 𝗱'𝘂𝘁𝗶𝗹𝗶𝘀𝗮𝘁𝗲𝘂𝗿:</b> @{username}\n"
    message_out_str += f"<b>💠 𝗣𝗿𝗼𝗳𝗶𝗹:</b> <a href='tg://user?id={from_user.id}'><b>ᴄʟɪᴄᴋ ʜᴇʀᴇ</b></a>\n"
    if message.chat.type in ((enums.ChatType.SUPERGROUP, enums.ChatType.CHANNEL)):
        try:
            chat_member_p = await message.chat.get_member(from_user.id)
            joined_date = (chat_member_p.joined_date or datetime.now()).strftime("%Y.%m.%d %H:%M:%S")
            message_out_str += f"<b>💠 𝗔 𝗿𝗲𝗷𝗼𝗶𝗻𝘁 𝗹𝗮 𝗱𝗶𝘀𝗰𝘂𝘀𝘀𝗶𝗼𝗻 𝗹𝗲:</b> <code>{joined_date}</code>\n"
        except UserNotParticipant: pass
    chat_photo = from_user.photo
    if chat_photo:
        local_user_photo = await client.download_media(message=chat_photo.big_file_id)
        buttons = [[InlineKeyboardButton('❎ 𝐅𝐞𝐫𝐦𝐞𝐫', callback_data='close_data')]]
...
async def imdb_search(client, message):
    if ' ' in message.text:
        k = await message.reply('Recherche en cours....')
        r, title = message.text.split(None, 1)
        movies = await get_poster(title, bulk=True)
        if not movies:
            return await message.reply("Aucun résultat.")
        btn = [[InlineKeyboardButton(f"{movie.get('title')} - {movie.get('year')}", callback_data=f"imdb#{movie.movieID}")] for movie in movies ]
        await k.edit('Voici le(s) résultat(s) de la requête.', reply_markup=InlineKeyboardMarkup(btn))
    else:
        await message.reply('Envoie-moi un sujet à rechercher sur IMDb')
