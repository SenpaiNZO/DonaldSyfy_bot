...
async def answer(bot, query):
    ...
    if AUTH_CHANNEL and not await is_subscribed(bot, query):
        await query.answer(results=[],
                           cache_time=0,
                           switch_pm_text='Vous devez vous abonner à mon canal VIP.',
    ...
    if results:
        switch_pm_text = f"{emoji.FILE_FOLDER} Résultats de la requête - {total}"
        if string:
        ...
        switch_pm_text = f'{emoji.CROSS_MARK} Pas de résultats'
        if string:
            switch_pm_text += f' for "{string}"'
...
def get_reply_markup(query):
    buttons = [[InlineKeyboardButton('♻️ Rechercher à nouveau', switch_inline_query_current_chat=query)]]
