...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        else: await query.answer("Vous avez besoin d'être propriétaire du groupe pour le faire.", show_alert=True)
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            else: await query.answer("Poto arrête, tu n'y as pas accès. 😁", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        files_ = await get_file_details(file_id)
        if not files_: return await query.answer('Aucune fichier existant.')
        files = files_[0]
        title = files.file_name
        size = get_size(files.file_size)
        f_caption = f_caption = f"{title}"
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('Vérifie en MP, je t\'ai envoyé le fichier', show_alert=True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            return await query.answer("Désolé(e) l'ami(e), tu n'es toujours pas membre de mon canal VIP. Pour devenir membre de mon canal VIP, écris à [Ｓｅｎｐａｉ](https://t.me/the_boyfriend) et fais un don de 6€ (7$) (3900 FCFA). Un serveur de bot s'achète, donc contribue, bro. 😏", show_alert=True)
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_: return await query.answer('Fichier inexistant....')
        files = files_[0]
        title = files.file_name
        size = get_size(files.file_size)
        f_caption = f_caption = f"{title}"
        ...
        await query.answer("🤨 Tu es trop curieux, Wallah. 😁", show_alert=True)
        ...
        buttons = [[
            InlineKeyboardButton('⚙️ 𝗣𝗮𝗻𝗲𝗹 𝗱\'𝗮𝗱𝗺𝗶𝗻𝗶𝘀𝘁𝗿𝗮𝘁𝗲𝘂𝗿 ⚙️', 'admin')
            ],[
            InlineKeyboardButton('𝗙𝗶𝗹𝘁𝗿𝗲𝘀', 'openfilter'),
            InlineKeyboardButton('𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗿', 'coct')
            ],[
            InlineKeyboardButton('𝗦𝘁𝗼𝗰𝗸 𝗱𝗲 𝗳𝗶𝗰𝗵𝗶𝗲𝗿𝘀', 'newdata'),
            InlineKeyboardButton('𝗘𝘅𝘁𝗿𝗮-𝗠𝗼𝗱𝗲𝘀', 'extmod')
            ],[
            InlineKeyboardButton('𝗚𝗲𝘀𝘁𝗶𝗼𝗻 𝗱𝗲 𝗴𝗿𝗼𝘂𝗽𝗲𝘀', 'gpmanager'),
            InlineKeyboardButton('É𝘁𝗮𝘁 𝗱𝘂 𝗕𝗢𝗧 ❄️', 'stats')
            ],[
            InlineKeyboardButton('❎ 𝐅𝐞𝐫𝐦𝐞𝐫', 'close_data'),
            InlineKeyboardButton('⬅️ 𝐑𝐞𝐭𝐨𝐮𝐫', 'start')
        ]]
        ...
        buttons= [[
            InlineKeyboardButton('📚 𝙎𝙤𝙪𝙧𝙘𝙚 𝘾𝙤𝙙𝙚 📚', 'source')
            ],[
            InlineKeyboardButton('❎ 𝐅𝐞𝐫𝐦𝐞𝐫', 'close_data'),
            InlineKeyboardButton('⬅️ 𝐑𝐞𝐭𝐨𝐮𝐫', 'start')
        ]]
        ...
        buttons = [[
            InlineKeyboardButton('📚 𝙎𝙤𝙪𝙧𝙘𝙚 𝘾𝙤𝙙𝙚 📚', url='https://t.me/the_boyfriend')
            ],[
            InlineKeyboardButton('⬅️ 𝐑𝐞𝐭𝐨𝐮𝐫', 'about')
        ]]
        ...
        buttons = [[
            InlineKeyboardButton('❎ 𝐅𝐞𝐫𝐦𝐞𝐫', 'close_data'),
            InlineKeyboardButton('⬅️ 𝐑𝐞𝐭𝐨𝐮𝐫', 'help')
        ]]
        if query.from_user.id not in ADMINS:
            return await query.answer("𝐃é𝐬𝐨𝐥é 𝐜𝐞 𝐦𝐞𝐧𝐮 𝐞𝐬𝐭 𝐫é𝐬𝐞𝐫𝐯é 𝐮𝐧𝐢𝐪𝐮𝐞𝐦𝐞𝐧𝐭 à 𝐦𝐨𝐧 𝐚𝐝𝐦𝐢𝐧 ⚒️", show_alert=True)
        ...
        buttons = [[
            InlineKeyboardButton('❎ 𝐅𝐞𝐫𝐦𝐞𝐫', 'close_data'),
            InlineKeyboardButton('⬅️ 𝐑𝐞𝐭𝐨𝐮𝐫ᴋ', 'openfilter')
        ]]
        ...
        buttons = [[
            ...
            InlineKeyboardButton('❎ 𝐅𝐞𝐫𝐦𝐞𝐫', 'close_data'),
            InlineKeyboardButton('⬅️ 𝐑𝐞𝐭𝐨𝐮𝐫', 'openfilter')
        ]]
        ...
        buttons = [[
            InlineKeyboardButton('❎ 𝐅𝐞𝐫𝐦𝐞𝐫', 'close_data'),
            InlineKeyboardButton('⬅️ 𝐑𝐞𝐭𝐨𝐮𝐫', f"{'manuelfilter' if query.data == 'button' else 'globalfilter'}")
        ]]
        ...
        buttons = [[
            InlineKeyboardButton('♻️ 𝐀𝐜𝐭𝐮𝐚𝐥𝐢𝐬𝐞𝐫', 'stats'),
            InlineKeyboardButton('⬅️ 𝐑𝐞𝐭𝐨𝐮𝐫', 'help')
        ]]
        ...
        free = get_size(free)
        await query.message.edit('Chargement....')
        ...
        if str(grp_id) != str(grpid):
            return await query.message.edit("Votre connexion active a été modifiée. Allez à /settings .")
